...
static const int Unassigned = -1; // edge not currently 'owning' a solution
static const int Skip = -2;       // edge that would otherwise close a path
...
inline long long Round(double val) {
    if ((val < 0)) {
        return static_cast<long long>(val - 0.5);
    } else {
        return static_cast<long long>(val + 0.5);
    }
}
...
public:
    ...
    Int128(long long _lo = 0) {
        lo = static_cast<unsigned long long>(_lo);
        hi = _lo < 0 ? -1 : 0;
    }
    ...
    Int128(const long long& _hi, const unsigned long long& _lo) : lo(_lo), hi(_hi) {
    }

    Int128& operator=(const long long& val) {
        lo = (unsigned long long) val;
        if (val < 0)
            hi = -1;
        else
            hi = 0;
...
Int128 Int128Mul(long long lhs, long long rhs) {
    bool negate = (lhs < 0) != (rhs < 0);

    if (lhs < 0)
        lhs = -lhs;
    auto int1Hi = static_cast<unsigned long long>(lhs) >> 32ull;
    auto int1Lo = static_cast<unsigned long long>(lhs & 0xFFFFFFFF);

    if (rhs < 0)
        rhs = -rhs;
    auto int2Hi = static_cast<unsigned long long>(rhs) >> 32ull;
    auto int2Lo = static_cast<unsigned long long>(rhs & 0xFFFFFFFF);

    // nb: see comments in clipper.pas
    auto a = int1Hi * int2Hi;
    auto b = int1Lo * int2Lo;
    auto c = int1Hi * int2Lo + int1Lo * int2Hi;

    Int128 tmp;
    tmp.hi = static_cast<long long>(a + (c >> 32ull));
    tmp.lo = static_cast<long long>(c << 32ull);
    tmp.lo += static_cast<long long>(b);
    if (tmp.lo < b)
        tmp.hi++;
    if (negate)
        tmp = -tmp;
...
inline long long TopX(TEdge& edge, const long long currentY) {
    return (currentY == edge.Top.Y) ? edge.Top.X : edge.Bot.X + std::round(edge.Dx * (currentY - edge.Bot.Y));
}
...
void IntersectPoint(TEdge& Edge1, TEdge& Edge2, IntPoint& ip) {
    ...
    if (Edge1.Dx == Edge2.Dx) {
        ...
        if (IsHorizontal(Edge2))
            ...
            ip.Y = static_cast<long long>(std::round(static_cast<double>(ip.X) / Edge2.Dx + b2));
        }
    } else if (Edge2.Dx == 0) {
        ip.X = Edge2.Bot.X;
...
bool HorzSegmentsOverlap(long long seg1a, long long seg1b, long long seg2a, long long seg2b) {
    if (seg1a > seg1b) {
...
void RangeTest(const IntPoint& Pt, bool& useFullRange) {
    constexpr long long loRange = 0x3FFFFFFFll;
    constexpr long long hiRange = 0x3FFFFFFFFFFFFFFFll;
    if (useFullRange) {
...
TEdge* ClipperBase::ProcessBound(TEdge* E, bool NextIsForward) {
    ...
    if (IsHorizontal(*E)) {
        ...
        EStart = NextIsForward ? E->Prev : E->Next;
...
bool ClipperBase::AddPath(const std::vector<IntPoint>& pg, PolyType PolyTyp, bool Closed) {
...
bool ClipperBase::AddPaths(const std::vector<std::vector<IntPoint>>& ppg, PolyType PolyTyp, bool Closed) {
    bool result = false;
...
bool ClipperBase::PopLocalMinima(long long Y, const LocalMinimum*& locMin) {
...
void ClipperBase::InsertScanbeam(long long Y) {
    m_Scanbeam.push(Y);
}

bool ClipperBase::PopScanbeam(long long& Y) {
    if (m_Scanbeam.empty())
        return false;
    Y = m_Scanbeam.top();
    m_Scanbeam.pop();
...
bool Clipper::Execute(ClipType clipType, std::vector<std::vector<IntPoint>>& solution, PolyFillType subjFillType, PolyFillType clipFillType) {
    if (m_ExecuteLocked) {
        return false;
    }
    if (m_HasOpenPaths) {
...
OutPt* Clipper::AddLocalMinPoly(TEdge* e1, TEdge* e2, const IntPoint& Pt) {
    ...
    if (prevE && prevE->OutIdx >= 0 && prevE->Top.Y < Pt.Y && e->Top.Y < Pt.Y) {
        long long xPrev = TopX(*prevE, Pt.Y);
        long long xE = TopX(*e, Pt.Y);
...
void Clipper::InsertLocalMinimaIntoAEL(long long botY) {
...
void Clipper::IntersectEdges(TEdge* e1, TEdge* e2, IntPoint& Pt) {
    ...
    if (e1->WindDelta == 0 || e2->WindDelta == 0) {
        // ignore subject-subject open path intersections UNLESS they are both open paths, AND they are both 'contributing maximas' ...
        if (e1->WindDelta == 0 && e2->WindDelta == 0)
            return;
...
inline bool IsMaxima(TEdge* e, long long Y) {
    return e && e->Top.Y == Y && !e->NextInLML;
}

inline bool IsIntermediate(TEdge* e, long long Y) {
...
void GetHorzDirection(TEdge& HorzEdge, Direction& Dir, long long& Left, long long& Right) {
...
bool Clipper::ProcessIntersections(long long topY) {
    if (!m_ActiveEdges)
        return true;
    try {
...
void Clipper::BuildIntersectList(long long topY) {
    if (!m_ActiveEdges)
        return;
...
void Clipper::ProcessEdgesAtTopOfScanbeam(long long topY) {
    TEdge* e = m_ActiveEdges;
    while (e) {
...
void Clipper::BuildResult(std::vector<std::vector<IntPoint>>& polys) {
    polys.reserve(m_PolyOuts.size());
    for (auto& polyOut : m_PolyOuts) {
        if (!polyOut->Pts) {
            continue;
        }
        std::vector<IntPoint> pg;
        OutPt* p = polyOut->Pts->Prev;
...
bool GetOverlap(long long a1, long long a2, long long b1, long long b2, long long& Left, long long& Right) {
    if (a1 < a2) {
        if (b1 < b2) {
            Left = std::max(a1, b1);
            Right = std::min(a2, b2);
...
void ClipperOffset::AddPath(const std::vector<IntPoint>& path, JoinType joinType, EndType endType) {
    ...
    if (m_lowest.X >= 0 &&
        ...
        for (int i = 0; i < m_polyNodes.ChildCount(); ++i) {
            ...
            if (node.m_endtype == etClosedPolygon || (node.m_endtype == etClosedLine && Orientation(node.Contour))) {
                std::ranges::reverse(node.Contour);
            }
        }
    } else {
        for (int i = 0; i < m_polyNodes.ChildCount(); ++i) {
...
void ClipperOffset::Execute(std::vector<std::vector<IntPoint>>& solution, double delta) {
    solution.clear();
    FixOrientations();
    DoOffset(delta);
    Clipper clpr;
    ...
    if (delta > 0) {
        ...
        const std::vector<IntPoint> outer{
    ...
    for (int i = 0; i < m_polyNodes.ChildCount(); i++) {
        ...
        if (len == 1) {
            if (node.m_jointype == jtRound) {
                ...
                for (long long j{ 1 }; j <= steps; j++) {
                    m_destPoly.emplace_back(std::round(m_srcPoly[0].X + X * delta), std::round(m_srcPoly[0].Y + Y * delta));
                    double X2 = X;
                    X = X * m_cos - m_sin * Y;
                    Y = X2 * m_sin + Y * m_cos;
                }
        ...
        if (node.m_endtype == etClosedPolygon) {
            ...
            if (node.m_endtype == etOpenButt) {
                int j = len - 1;
                pt1 = IntPoint((long long) Round(m_srcPoly[j].X + m_normals[j].X * delta), (long long) Round(m_srcPoly[j].Y + m_normals[j].Y * delta));
            ...
            if (node.m_endtype == etOpenButt) {
                pt1 = IntPoint((long long) Round(m_srcPoly[0].X - m_normals[0].X * delta), (long long) Round(m_srcPoly[0].Y - m_normals[0].Y * delta));
                m_destPoly.push_back(pt1);
                pt1 = IntPoint((long long) Round(m_srcPoly[0].X + m_normals[0].X * delta), (long long) Round(m_srcPoly[0].Y + m_normals[0].Y * delta));
                m_destPoly.push_back(pt1);
            } else {
...
void ClipperOffset::OffsetPoint(int j, int& k, JoinType jointype) {
    ...
    if (std::fabs(m_sinA * m_delta) < 1.0) {
        ...
        if (cosA > 0) {
            m_destPoly.emplace_back(Round(m_srcPoly[j].X + m_normals[k].X * m_delta), Round(m_srcPoly[j].Y + m_normals[k].Y * m_delta));
            return;
        }
...
void ClipperOffset::DoSquare(int j, int k) {
    ...
    m_destPoly.emplace_back(Round(m_srcPoly[j].X + m_delta * (m_normals[k].X - m_normals[k].Y * dx)), Round(m_srcPoly[j].Y + m_delta * (m_normals[k].Y + m_normals[k].X * dx)));
    m_destPoly.emplace_back(Round(m_srcPoly[j].X + m_delta * (m_normals[j].X + m_normals[j].Y * dx)), Round(m_srcPoly[j].Y + m_delta * (m_normals[j].Y - m_normals[j].X * dx)));
}

void ClipperOffset::DoMiter(int j, int k, double r) {
    double q = m_delta / r;
    m_destPoly.emplace_back(Round(m_srcPoly[j].X + (m_normals[k].X + m_normals[j].X) * q), Round(m_srcPoly[j].Y + (m_normals[k].Y + m_normals[j].Y) * q));
}
...
void ClipperOffset::DoRound(int j, int k) {
    ...
    for (int i = 0; i < steps; ++i) {
        m_destPoly.emplace_back(Round(m_srcPoly[j].X + X * m_delta), Round(m_srcPoly[j].Y + Y * m_delta));
        X2 = X;
        X = X * m_cos - m_sin * Y;
        Y = X2 * m_sin + Y * m_cos;
    }
